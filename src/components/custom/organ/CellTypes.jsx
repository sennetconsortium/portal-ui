...
function CellTypes({organ, setShowCellTypesSide}) {
    ...
    const columns = [
        {
            ...
            sortable: true,
            reorder: true,
            format: row => <a href={getHotLink(row)}><span>{row.cell_label}</span><br/><small
                className='text-muted'>{row.cl_id}</small></a>,
        },
        {
            name: 'Description',
            id: 'cell_definition',
        ...
        },
        {
            ...
            sortable: false,
            reorder: true,
            format: row => <span role='button' className='btn btn-outline-primary js-btn--cellTypes__viewDatasets'
                                 onClick={(e) => getIds(e, row)}>View Datasets</span>,
        }
    ]
        ...
        <>
          ...
          >
            Download Table Data <i className="bi bi-download"></i>
                    ...
                    {chartData && <div className='position-relative'><CellTypeDistributionAcrossOrgansTab
                        organ={{...organ, _id: organId}} tabData={chartData} cell={{cellIds: cellIds.current}}/></div>}
