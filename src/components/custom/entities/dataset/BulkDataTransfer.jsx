...
function BulkDataTransfer({data}) {
    ...
    const contactUs = <LnkIc title={'contact us'} href={'mailto:help@sennetconsortium.org'} icClassName={'bi bi-envelope-fill'} />
    ...
    const tooltip = (id = '0', def = true) => (
        <SenNetPopover text={<span>{def ? 'Global research data management system.' : toolTopDbGap}</span>} className={`popover-${id}`}>
    ...
    const notLoggedIn = (entity) => {
        return (
            <>
                ...
                <p>Please <a href={'/login'}>log in</a> for Globus access or {contactUs} with the <code>Dataset</code> ID about the files you are trying to access.</p>
                <hr />
                <h6>Non-Consortium Members: Database of Genotypes and Phenotypes (dbGaP){tooltip(entity.uuid + '1',false)}</h6>
                <p>This <code>Dataset</code> contains protected-access human sequence data. Data is not yet available through dbGap , but will be available soon.
    ...
    const loggedInNoAccess = (entity) => {
        return (<>
            ...
            <p>This <code>Dataset</code> contains protected-access human sequence data. Please ask the PI of your SenNet award to&nbsp;
    ...
    const loggedInAccess = (entity) => {
        return (<>
            ...
            <p><LnkIc href={entity.globusPath} title={`${entity.dataset_type} (${entity.sennet_id})`}/></p>
        </>)
    }
