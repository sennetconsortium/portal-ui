...
function AssociatedEntity({ data, currentEntity, grammar = 'is contained in the' }) {
    return (
        <SenNetAccordion id={`AssociatedEntity--${data.entity_type}`} title={`Associated ${data.entity_type}`}>
            <Card border='0'>
                <Card.Body>
                    <p className='fw-light fs-6'>This <code>{currentEntity}</code> {grammar} <code>{data.entity_type}</code>&nbsp;
